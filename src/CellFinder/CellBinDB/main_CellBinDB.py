...
def get_args_parser():
    ...
    parser.add_argument('--finetune', default='/home/share/huadjyin/home/zhangzhenke/Code/models/temp/00_Phase2_CellBinDB/checkpoint-59.pth',
    ...
    parser.add_argument('--data_path', default='./data', type=str,
                        help='dataset path')
    # 结果
    parser.add_argument('--output_dir', default='./temp/02/',
                        help='path where to save, empty for no saving')
    # 日志
    parser.add_argument('--log_dir', default='./temp/02/',
...
def main(args):
    ...
    dataset_train = DatasetTrain(cityscapes_data_path="./data/zzk/CellBinDB/",
                             cityscapes_meta_path="./data/zzk/CellBinDB/")
    print(dataset_train)
    dataset_val = DatasetVal(cityscapes_data_path="./data/zzk/CellBinDB/",
                             cityscapes_meta_path="./data/zzk/CellBinDB/")
    print(dataset_val)


    num_train_batches = int(len(dataset_train)/args.batch_size)
    num_val_batches = int(len(dataset_val)/1)
    ...
    # 实例化模型
    model = CellFinder.__dict__[args.model](        
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        confmat, confmat1, confmat2, confmat3, confmat4, confmat5  = evaluate(model, data_loader_val, device=device)
        val_info = str(confmat)
        val_info1 = str(confmat1)
        val_info2 = str(confmat2)
        val_info3 = str(confmat3)
        val_info4 = str(confmat4)
        val_info5 = str(confmat5)


        # 使用正则表达式提取数字
        match = re.search(r'\d+\.\d+', val_info)
        match1 = re.search(r'\d+\.\d+', val_info1)
        match2 = re.search(r'\d+\.\d+', val_info2)
        match3 = re.search(r'\d+\.\d+', val_info3)
        match4 = re.search(r'\d+\.\d+', val_info4)
        match5 = re.search(r'\d+\.\d+', val_info5)
        if match:
            number = float(match.group())
            number1 = float(match1.group())
            number2 = float(match2.group())
            number3 = float(match3.group())
            number4 = float(match4.group())
            number5 = float(match5.group())
        else:
        ...
        if args.rank in [-1, 0]:
            ...
            with open(os.path.join(args.output_dir, "Pr.txt"), mode="a", encoding="utf-8") as f:
                f.write(str(epoch) + "  " + str(number1) + "\n")
            with open(os.path.join(args.output_dir, "Re.txt"), mode="a", encoding="utf-8") as f:
                f.write(str(epoch) + "  " + str(number2) + "\n")
            with open(os.path.join(args.output_dir, "AP.txt"), mode="a", encoding="utf-8") as f:
                f.write(str(epoch) + "  " + str(number3) + "\n")
            with open(os.path.join(args.output_dir, "DQ.txt"), mode="a", encoding="utf-8") as f:
                f.write(str(epoch) + "  " + str(number4) + "\n")
            with open(os.path.join(args.output_dir, "PQ.txt"), mode="a", encoding="utf-8") as f:
                f.write(str(epoch) + "  " + str(number5) + "\n")
    ...
    output_path = os.path.join(args.output_dir, 'cellbindb_f1.png')
